 ...
 */
        ...
        if(car2._touchDeltaY<=0){
 		...
 		function init_next(node){
	        ...
	        if(car2._movePosition=='up'){
 				...
 				node[0].style[car2._prefixStyle('transform')] = 'translate(0,-'+Math.max(200,$(node[0]).height())+'px)'+_translateZ;
 				$(node[0]).attr('data-translate',-Math.max(200,$(node[0]).height()));
 	...
 	page_success	: function(){
 		...
 		// 当前页面变小的移动
 		var y = car2._touchDeltaY > 0 ? 200/5 : -200/5;
 		var scale = 0.8;
 	...
 	page_fial	: function(){
		...
		if(!car2._pageNext&&car2._pageNext!=0) {
 		...
 		if(car2._movePosition=='up'){
 			car2._page.eq(car2._pageNext)[0].style[car2._prefixStyle('transform')] = 'translate(0,'+200+'px)'+_translateZ;
 		}else{
 			car2._page.eq(car2._pageNext)[0].style[car2._prefixStyle('transform')] = 'translate(0,-'+200+'px)'+_translateZ;
 		}
 ...
 */
	...
	styleInit : function(){
		...
		$('.p-ct').height(246);
		$('.m-page').height(246);
		$('#j-mengban').height(246);
		$('.translate-back').height(246);
	},
